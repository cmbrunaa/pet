...
function verificarConsumoAnormal(
  consumoHoje,
  mediaDiaria
){
  ...
  return new Promise((resolve,reject)=>{
    ...
    db.query(query,[usuarioId],(err,rows)=>{
      ...
      rows.forEach(item=>{
        ...
        const index =
          ultimos7Dias.findIndex(
            d=>d.dia === diaBanco
          );

        if(index !== -1){

          ultimos7Dias[index].total =
            Number(item.total);
  ...
  return new Promise((resolve,reject)=>{
    ...
    db.query(query,[usuarioId],(err,rows)=>{
      ...
      const totalRegistros =
        Number(dados.totalRegistros) || 0;

      const mediaDiaria =
        totalRegistros > 0
          ? Number(
              (totalConsumido / totalRegistros)
              .toFixed(2)
            )
          : 0;

      const consumoHoje =
        dados.menorConsumo || 0;

      // 🚨 IA AQUI

      const alertaConsumo =
        verificarConsumoAnormal(
          consumoHoje,
          mediaDiaria
        );
      ...
      resolve({
        ...
        maiorConsumo:
          Number(dados.maiorConsumo) || 0,

        menorConsumo:
          Number(dados.menorConsumo) || 0,
